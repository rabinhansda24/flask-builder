...
function createModule(appDir: string, name: string = "") {
  if (!name) {
    return;
  }
...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log(
    'Congratulations, your extension "create-flask-app" is now active!',
  );

  // The command has been defined in the package.json file
  // Now provide the implementation of the command with registerCommand
  // The commandId parameter must match the command field in package.json
  let createPythonModuleCommand = vscode.commands.registerCommand(
  ...
  let createFlaskAppCommand = vscode.commands.registerCommand(
  ...
  let createFlaskModuleCommand = vscode.commands.registerCommand(
    "extension.createFlaskModule",
    async (uri: vscode.Uri | undefined) => {
      if (!uri) {
        const folderUris = await vscode.window.showOpenDialog({
          canSelectFolders: true,
  ...
  context.subscriptions.push(createFlaskModuleCommand);
}
